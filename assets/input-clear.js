// Transitions.dev — Input clear with dissolve (POSTERLEAGUE)
(function() {
      ...
      text.split(/(\s+)/).forEach(function(seg) {
        ...
        if (seg.trim()) {
          ...
          [[0, 0.8, 7, 0.22], [hw * 0.45, 0.55, 8, 0.18],
            [-hw * 0.4, 0.65, 6, 0.16], [hw * 0.15, 0.9, 5, 0.14]].forEach(function(pair) {
            const dx = pair[0];
            const rwm = pair[1];
            const rh = pair[2];
            const a = pair[3];
  ...
  document.addEventListener('DOMContentLoaded', function() {
    document.querySelectorAll('.t-clear').forEach(initTClear);
  });

  window.plInitTClear = initTClear;
})();
